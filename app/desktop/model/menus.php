...
class desktop_mdl_menus extends dbeav_model{
   ...
   function menu($get,$defaultWorkground){ 
        ...
        //根据version_code处理菜单
        kernel::single('taocrm_system')->set_system_menu($menu,'left_pannel');  
        ...
        foreach($menu_group as $key_=>$value){
            ...
            foreach($menu as $res){
                ...
                if(strstr($_SERVER['SERVER_NAME'], 'mcrm.taoex.com') 
                    && strstr(',RF分析,RFM分析,销售漏斗,积分日志,兑换优惠券,兑换日志,运营决策,积分规则,赠品列表,赠品规则,', $res['menu_title']))
                        continue;
    ...
    * 删除左边某个菜单栏
    */
   public function subMenuOption(&$menu)
   {
       foreach ($menu as &$menuTitle1) {
            foreach ($menuTitle1['menu'] as $menutKey => $menuTitle2) {
                $appParams = explode("&", $menuTitle2['menu_path']);
                ...
                if (is_object($obj) && method_exists($obj, $act)) {
                    if ($obj->$act() == false) {
                        unset($menuTitle1['menu'][$menutKey]);
                    }
                }
                unset($funcItem);
            }
