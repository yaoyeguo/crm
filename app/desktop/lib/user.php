...
class desktop_user{
   ...
   function get_work_menu(){
        ...
        //根据version_code处理菜单
        kernel::single('taocrm_system')->set_system_menu($aWorkground, 'top_menu');
        kernel::single('taocrm_system')->set_system_menu($aMenu, 'top_sub_menu');
        ...
        foreach((array)$aData['menu'] as $k1=>$group){
            if(!in_array($k1,(array)$allkey_workground)) continue;
        ...
        $this->chk_menu_version($aData);
        $this->subMenuOption($aData);
        return $aData;
    }
   
    //区分线上线下的不同菜单
    public function chk_menu_version(&$menu)
    {
        if(strstr($_SERVER['SERVER_NAME'], 'mcrm.taoex.com')){
            unset($menu['workground']['taocrm.sales']);
            unset($menu['menu']['taocrm.analysts']['客户分析'][3]);
            unset($menu['menu']['taocrm.analysts']['客户分析'][4]);
            unset($menu['menu']['taocrm.analysts']['客户分析'][5]);
            unset($menu['menu']['taocrm.member']['客户管理'][1]);
            unset($menu['menu']['market.sales']['优惠券管理'][2]);
            unset($menu['menu']['market.sales']['优惠券管理'][3]);
            unset($menu['menu']['taocrm.analysts']['经营概况'][5]);
            unset($menu['menu']['sys.config']['基本设置'][4]);
            unset($menu['menu']['sys.config']['基本设置'][5]);
            unset($menu['menu']['sys.config']['基本设置'][6]);
        }
    }
    
    //检查菜单
    public function subMenuOption(&$menus)
    {
        foreach ($menus['menu'] as &$menuTitle1) {
            foreach ($menuTitle1 as $menutKey => &$menuTitle2) {
                foreach ($menuTitle2 as $menutKey3 => $menuTitle3) {
                    $appParams = explode("&", $menuTitle3['menu_path']);
                    foreach ($appParams as $item) {
                        list( , $funcItem[]) = explode("=", $item);
                    }
                    list($app, $ctl, $act) = $funcItem;
                    $serviceName = 'submenu_' . $app . '_' . $ctl;
                    $obj = kernel::service($serviceName);
                    if (is_object($obj) && method_exists($obj, $act)) {
                        if ($obj->$act() == false) {
                            unset($menuTitle2[$menutKey3]);
                        }
                    }
                    unset($funcItem);
                }
        ...
        //获得wangwang与TaoCrm绑定状态
//        $bind = false;
//        if ($bind == false) {
//            if (isset($menus['menu']['taocrm.member']['客户管理'][2]) && $menus['menu']['taocrm.member']['客户管理'][2]['menu_title'] == '旺旺属性客户') {
//                //$menus['menu']['taocrm.member']['客户管理'][2] == null;
//                unset($menus['menu']['taocrm.member']['客户管理'][2]);
//            }
//        }
    }
