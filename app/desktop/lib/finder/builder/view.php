...
class desktop_finder_builder_view extends desktop_finder_builder_prototype{
    ...
    function _script(){
        ...
        $finderOptions = array(
            ...
            'packet'=>$data&&$this->use_view_tab,
        );
        //if($finderOptions['packet']) $finderOptions['packet'] = (count($data)>0)?true:false;
        /** 判断是否要显示归类视图 **/
        $is_display_packet = 'false';
        if ($finderOptions['packet']){
            foreach ($data as $arr){
                if ($arr['addon'] or $arr['display']){
...
EOF;
    ...
    array('label'=>app::get('desktop')->_('为选中项打标签'),'submit'=>$this->url.'&action=tag','target'=>'dialog::{width:400,title:\''.app::get('desktop')->_('设置标签').'\'}')
            ...
            if($this->has_tag==true&&$this->use_buildin_tagedit){
                ...
                else { $_tagediturl='index.php?app=desktop&ctl=default&act=alertpages&nobuttion=1&goto='.urlencode('index.php?app='.$this->app->app_id.'&ctl='.$_GET['ctl'].'&act=tags&nobuttion=1&type='.$this->short_object_name);
                }
     array_push($_tagaction['group'],array('label'=>'_SPLIT_'),array('label'=>app::get('desktop')->_('标签设置'),'href'=>$_tagediturl,'target'=>'_blank'));
        ...
        $render->pagedata['haspacket'] =  $use_view_tab_data&&$this->use_view_tab;
        //if($render->pagedata['haspacket']) $render->pagedata['haspacket'] = (count($this->get_views())>0)?true:false;
        /** 判断是否要显示归类视图 **/
        $is_display_packet = 'false';
        foreach ($use_view_tab_data as $arr){
            if ($arr['addon'] or $arr['display'])
            {
                $is_display_packet = 'true';
                break;
...
EOF;
        ...
        if($this->params===-1){
            ...
            $list = $this->object->$getlist_method(implode(',',$sql),$this->params,($page-1)*$this->pagelimit,$this->pagelimit,$order,$this->force_index);
