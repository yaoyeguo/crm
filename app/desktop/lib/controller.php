...
class desktop_controller extends base_controller{
    ...
    function __construct($app){
        ...
        if(get_class($this)!='desktop_ctl_passport' && !$account->is_valid()){
            ...
            echo "<script>
					void function(){
						var _GOTO ='index.php?ctl=passport&url=".$url."&params=".$params."';
						/*if(opener){opener.location = _GOTO;window.close();return;}*/
						location = _GOTO;
				     }();
				 </script>";
            exit;
        ...
        if($_GET['ctl']!="passport"&&$_GET['ctl']!=""){
            ...
            if(!$this->status&&$this->status==0){
                ...
                echo "<script>
						void function(){
							var _GOTO ='index.php?ctl=passport&act=logout';
							/*if(opener){opener.location = _GOTO;window.close();ereturn;}*/
							top.location = _GOTO;
					     }();
					 </script>";
                exit;
        ...
        if(app::get('bizcloud')->is_actived()){
            $this->pagedata['default_toolbar_header_top'] = '$extend(pos,{left:fcis.left,top:fcis.bottom-35});';
        }
    }

    function initCrmVersion(){
        ...
        //替换图片地址
        $logo_title = str_ireplace('img src="','img src="'.kernel::base_url(true).'/',$logo_title);   
        $passport_img = '<iframe src="http://pmp.shopex.cn/index.php?app=ad&ctl=display&act=index&nobar=1&nobtn=1&code=42a614d3a228c25d638360c177086cd4&entid=131110483129&username=&r=1380435076" frameborder="0" scrolling="no" width="340px" height="240px" marginheight="0px" marginwidth="0px"></iframe>';
        $passport_img = '<img src="'.kernel::base_url(false).'/app/taocrm/statics/logo/256x256.gif" />';

        switch($crm_version){
            case '--SELF_ENT_DEPLOY':
            case '--SELF_FLAG_DEPLOY':
                $this->pagedata['passport_logo'] = '会员营销中心';
                $this->pagedata['passport_img'] = '<img src="'.app::get('taocrm')->res_url.'/ShopExCommerce.png">';
                $this->pagedata['default_logo'] = '会员营销中心';
                $this->pagedata['default_toolbar'] = '<iframe src="http://portal.id.shopex.cn/topbar.php"
				frameborder="0" scrolling="no" height="26" width="100%" marginheight="0px" marginwidth="0px"></iframe>';
                $this->pagedata['default_toolbar_header_top'] = '$extend(pos,{left:fcis.left,top:fcis.bottom-30});';
                break;
            default:
                $this->pagedata['passport_logo'] = $login_title;
                $this->pagedata['passport_logo_desc'] = $logo_desc;
                $this->pagedata['default_logo'] = $logo_title;
                $this->pagedata['passport_img'] = $passport_img;
                $this->pagedata['default_toolbar'] = '';
                $this->pagedata['default_toolbar_header_top'] = '$extend(pos,{left:fcis.left,top:fcis.bottom});';
                break;
        }
