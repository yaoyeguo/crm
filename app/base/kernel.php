...
class kernel{
     ...
     * 日志־
     */
    static public function ilog($str,$model='crontab') {
        if (defined('COMMAND_MODE') && COMMAND_MODE) {
            echo $str."\n";
            exit;
        }

        $_SERVER['SERVER_NAME'] = isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'localhost';
        $path = SCRIPT_DIR . '/logs/'.$model.'/'.$_SERVER['SERVER_NAME'];
        if (!is_dir($path)) {
    ...
    static public function setEntInfo($params) {
        app::get('base')->setConf('ecos.enterprise_info', serialize($params));
    }

    static public function getEntInfo() {
