...
class base_application_manage{
    ...
    static function content_detector($app_id=null, $type='all')
    {
        if($type == 'all'){
     ...
     * 仅更新app菜单和权限
     */
    public function update_app_menu($app_id)
    {
        $detectors = $this->content_detector($app_id, 'only_menu');
        if(!$detectors){
            return false;
        }
        foreach($detectors as $k=>$detector){
            $detector->update($app_id);
        }
    }
