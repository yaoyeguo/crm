...
class base_db_model implements base_interface_model{
    ...
    public $db_structs = array();
    public $no_recycle = false;
    ...
    {  
        if(!$filter){
            $cache_count = ecorder_func::get_total_count_cache($this->table_name(1));
            if($cache_count){
                return $cache_count;
            }
        }    
        ...
        //缓存结果
        if(!$filter) ecorder_func::set_total_count_cache($this->table_name(1), intval($row[0]['_count']));
    ...
    public function insert(&$data){
        ...
        if($sql && $this->db->exec($sql, $this->skipModifiedMark)){
            ...
            //缓存总数
            ecorder_func::add_total_count_cache($this->table_name(true), 1);
    ...
    public function get_structs(){
        ...
        if(!isset($this->db_structs[$table])){
            $structs = array();
            if($this->schema['columns']){
                foreach($this->schema['columns'] as $key=>$col){
                    if(isset($col['sdfpath']) && !empty($col['sdfpath'])){
                        $structs[$col['sdfpath']] = $key;
                    }else{
                        $structs[$key] = $key;
                    }
                }
                $this->db_structs[$table] = $structs;
            }
        }
        return $this->db_structs[$table];
    }

    function getPager($filter=null, $cols='*', $lim=0, $limit=1){
        $sql = 'select '.$cols.' FROM `'.$this->table_name(true).'` ';
        $whereSql = '';
        $limitSql = '';
        if ($filter) $whereSql .= " WHERE ".$this->_filter($filter);
        $limitSql .= " limit $lim,$limit ";
        $rows =  kernel::database()->select($sql . $whereSql . $limitSql);

        $selectField = " SELECT count(*) as counts FROM (".$sql.$whereSql.") c";
        $count = kernel::database()->select($selectField);

        return array('data'=>$rows,'count'=>$count[0]['counts']);
    }
