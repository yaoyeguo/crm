...
class ecorder_rpc_response_order extends ecorder_rpc_response {
    ...
    {
        if(defined('CRM_ENVIRONMENT') && CRM_ENVIRONMENT == 'docker'){
            $sdf['source'] = 'manual';
        }
        
        if($sdf['source'] == 'cstools'){
            base_kvstore::instance('taocrm')->fetch('ext_auth_secret_'.$sdf['secret_key'],$data);
            if($data){
                $data = json_decode($data,true);
                if($data['expire'] >= time()){
                    $sdf['f_modified'] = time();
                    kernel::single('taocrm_service_redis')->redis->RPUSH('tgcrm:SYS_CSTOOLS_ORDER_QUEUE',serialize(array('order' => $sdf, 'nodeId' => base_rpc_service::$node_id, 'host'=>$_SERVER['SERVER_NAME'])));
                    return array('tid' => $sdf['order_bn']);
                    //$order = new ecorder_rpc_response_order_add();
                    //return $order->add($sdf, $responseObj);
                }else{
                    return array('error' => 'Has expired');
                    //$responseObj->send_user_error(app::get('base')->_('Has expired'));
                }
            }else{
                return array('error' => 'Please sign in');
                //$responseObj->send_user_error(app::get('base')->_('Please sign in'));
            }
        }else{
           if($sdf['source'] == 'manual'){//后台下载订单
               $order = new ecorder_rpc_response_order_add();
               return $order->add($sdf, $responseObj);
           }else{
     ...
     * @return array('order_id'=>'订单主键ID')
     */
    public function add_back($sdf, &$responseObj) {

        //if ($_SERVER['SERVER_NAME'] == '65753061.taojixiao.taoshopex.com') {

        /*$queue = new ecorder_rpc_queue();
         $queue->push(array('order' => $sdf, 'nodeId' => base_rpc_service::$node_id, 'host'=>$_SERVER['SERVER_NAME']));
         return array('tid' => $sdf['order_bn']);
         //}
         exit;*/
        $order_sdf = $sdf;
        ...
        $order_sdf['weight'] = $order_sdf['weight'] ? $order_sdf['weight'] : 0.00;
        ...
        $order_sdf['createtime'] = kernel::single('ecorder_func')->date2time($order_sdf['createtime']); //订单创建时间
        $order_sdf['pay_time'] = kernel::single('ecorder_func')->date2time($payment_detail['pay_time']);		//支付时间
        $order_sdf['delivery_time'] = kernel::single('ecorder_func')->date2time($order_sdf['delivery_time']);	//发货时间
        $order_sdf['finish_time'] = kernel::single('ecorder_func')->date2time($order_sdf['finish_time']);		//交易完成时间
        ...
        foreach ($objects as $key => $object) {
            ...
            foreach ($items as $k => $item) {
                ...
                //$order_sdf['order_objects'][$key]['order_items'][$k]['create_time'] = $order_sdf['createtime'];
                //$order_sdf['order_objects'][$key]['order_items'][$k]['shop_id'] = $shop_id;
                ...
                if ($service = kernel::servicelist('service.order')){
                    foreach ($service as $object => $instance){
                        if (method_exists($instance, 'save_member')){
                            $member_id = $instance->save_member($order_sdf);
                        }
                    }
                }
    ...
    private function _save_member($member_info, $shop_id) {
        ...
        if ($member_info['uname']) {
            ...
            if(!$member_info['email'] && strstr($member_info['alipay_no'],'@')) $member_info['email'] = $member_info['alipay_no'];
            ...
            // ),
                    ...
                    'alipay_no' => $member_info['alipay_no'],
                    'zip' => $member_info['zip']
            //),
            );
            if (empty($member_detail['member_id'])) {
                //增加客户
                //echo "<pre>";print_r($members_data);
