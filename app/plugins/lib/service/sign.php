...
class plugins_service_sign{
    ...
    public function get_desc()
    {
        $conf_desc = array(
            'title'=>'签收提醒',
            'worker'=>__CLASS__,
            'desc'=>'【仅限淘宝店铺】买家签收宝贝后，自动发送短信提醒。短信发送时间：8:00-20:00',
            'icon'=>'sign.png',
            'price'=>array(0),
            'month'=>array(120),
            'status'=>'active',
            'addons'=>'tags',
            'tags'=>array('姓名','昵称','店铺','签收日期','物流公司'),
            'sms_template'=>'亲爱的<{姓名}>，您于<{签收日期}>签收的宝贝是否满意？满意请给我们点亮5颗星星哦。有问题随时联系我们~',
            'sort'=>3
        );
    ...
    public function get_items($sdf)
    {
        ...
        $conf_items = array(
            'days'=>array(
                'label'=>'签收后',
                'type'=>'select',
                'options'=>array(1=>'当天',2=>'第二天',3=>'第三天'),
                'default' => 1,
            ),
            'shop_id'=>array(
                'label'=>'有效店铺',
            ...
            'send_content'=>array(
                'label'=>'短信内容',
                'type'=>'textarea',
                'options'=>'亲爱的<{姓名}>，您于<{签收日期}>签收的宝贝是否满意？满意请给我们点亮5颗星星哦。有问题随时联系我们~ 退订回N【<{签名}>】'
            ),
        );
        return $conf_items;
    }
    ...
    public function run_hour($arr_param)
    {
        ...
        $id_arr = array();
        $sms = array();
        $sql = "select a.sign_time,a.tid,a.id,a.buyer_nick,a.ship_name,a.ship_mobile,a.shop_id,a.logi_company,a.logi_no from sdb_plugins_trades as a 
        where a.sms_status=0 and a.logi_status='2' and a.sign_time>$start_date and a.sign_time<=$end_date";
        //die($sql);
        $rs = $db->select($sql);
        if(!$rs) $rs = array();
        foreach($rs as $v){
            ...
            if(!in_array($v['ship_mobile'],$mobiles) && $v['ship_mobile']!='' 
                ...
                //'姓名','昵称','店铺','签收日期','物流公司'
                $sms_content = str_replace('<{签收日期}>',date('m.d',$v['sign_time']),$sms_content);
        ...
        if($sms){
            ...
            for($i=0;$i<=(sizeof($sms)/$this->sms_page_size);$i++){
                ...
                //err_log($content);
                $send_res = $oServiceApi->_send_sms($content,'cf'.$start_date.'b'.$i, '签收提醒');
                //if(!$send_res) break;
            }
    ...
    {    
        ...
        if(floatval($order_sdf['delivery_time']) < strtotime('-3 days')){
            return false;
        }
    ...
    public function trace_logistics()
    {
        $plugin_id = __CLASS__;
        $db = &kernel::database();
        $sql = 'select id,tid,seller_nick,city,sms_status,logi_status,sign_time from sdb_plugins_trades where logi_status in ("0","1") and update_time<'.(time() - 1800).' and ship_status=1 and order_status="active" limit 500';
        $rs = $db->select($sql);
        if(!$rs) return false;
        ...
        foreach($rs as $v){
            ...
            foreach($transit_step_info as $vv){
                ...
                if(
                    ...
                    if($v['logi_status']=='0') $sms_status = 0;
                    $logi_status = '1';
