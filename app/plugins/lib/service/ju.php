...
class plugins_service_ju{
    ...
    public function get_desc(){
        $conf_desc = array(
            'title'=>'聚划算催付',
            'worker'=>'plugins_service_ju',
            'desc'=>'【仅限淘宝店铺】聚划算专用插件。短信发送时间：8:00-20:00',
            'price'=>array(0),
            'month'=>array(120),
            'status'=>'active',
            'addons'=>'tags',
            'tags'=>array('姓名','昵称','店铺'),
            'sms_template'=>'亲爱的<{姓名}>，您的聚划算订单还没有支付哦，15分钟后就会自动关闭订单，我们已经给亲打包好了，亲付款后就可以领走了~',
            'view'=>'ju',
            'sort'=>11
        );
    ...
    public function get_items($sdf)
    {
        ...
        $conf_items = array(
            ...
            'send_content'=>array(
                'label'=>'短信内容',
                'type'=>'textarea',
                'options'=>'亲爱的<{姓名}>，您的聚划算订单还没有支付哦，15分钟后会自动关闭，我们已经给亲打包好了，快付款领走吧~ 退订回N【<{签名}>】'
            ),
        );
        return $conf_items;
    }
    ...
    // 每分钟运行插件
    public function run_task($arr_param)
    {
        ...
        //判断活动开始和结束时间
        $ju_start_time = strtotime($params['start_time']['date'].' '.$params['start_time']['hour'].':'.$params['start_time']['min'].':00');
        $ju_end_time = strtotime($params['end_time']['date'].' '.$params['end_time']['hour'].':'.$params['end_time']['min'].':00');

        //活动未开始
        if($ju_start_time>time()) return false;
        
        //活动结束了
        if($ju_end_time<time()) return false;
        
        //间隔几分钟没付款
        $send_time = intval($params['send_time']) * 60;
        
        //不满足催付时间间隔
        if((time() - $ju_start_time - 60) < $send_time) return false;
        
        //下单截止时间
        base_kvstore::instance('plugins')->fetch('order_end_time', $order_end_time);
        if(!$order_end_time) $order_end_time = $ju_start_time;

        //每次执行一分钟的数据
        $order_start_time = $order_end_time;
        //$order_start_time = time() - 86400;
        $order_end_time = time() - $send_time;
        base_kvstore::instance('plugins')->store('order_end_time', $order_end_time);
        ...
        if($send_time>0){
            ...
            foreach($params['shop_id'] as $v){
                $get_order_params = array(
                    'start_created' => date('Y-m-d H:i:s', $order_start_time),
                    'end_created' => date('Y-m-d H:i:s', $order_end_time),
                    'shop_info' => $shops[$v],
                    'status' => 'ALL_WAIT_PAY',
                    'type' => 'taobao',
                );
            ...
            if($rs){
                foreach($rs as $k=>$v){
                    ...
                    if($v['orders']['order']){
                        if(isset($v['orders']['order']['outer_iid'])){
                            $rs[$k]['bn'][] = $v['orders']['order']['outer_iid'];
                        }else{
                            foreach($v['orders']['order'] as $vv){
                                $rs[$k]['bn'][] = $vv['outer_iid'];
                            }
                        }
                    }
                }
