<?php
class plugins_service_reminder{
    ...
    public function get_desc(){
        $conf_desc = array(
            'title'=>'短信催付',
            'worker'=>'plugins_service_reminder',
            'desc'=>'帮助提高店铺的付款率，对超过指定时间仍未付款的订单，给买家发送提醒短信。短信发送时间：8:00-19:00',
            'icon'=>'mail.png',
            'price'=>array(0),
            'month'=>array(120),
            'status'=>'active',
    ...
    public function run_hour($arr_param)
    {
        ...
        and a.pay_status="0" and a.total_amount>='.floatval($params['min_amount']).' and a.status="active" and a.trade_type<>"step"
        ';
        $rs = $db->select($sql);
        if(!$rs) $rs = array();
        ...
        if($sms){
            ...
            for($i=0;$i<=(sizeof($sms)/$this->sms_page_size);$i++){            
                ...
                $send_res = $oServiceApi->_send_sms($content,'cf'.$now_date.'b'.$i, '短信催付');
