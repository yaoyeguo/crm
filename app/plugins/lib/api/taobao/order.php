...
class plugins_api_taobao_order extends plugins_api_taobao_request implements plugins_api_interface {
    /**
     * 通过API接口要获取的订单内容
     *
     * @var String
    ...
    const FIELDS_REFUND = 'refund_id, tid, title, buyer_nick, seller_nick, total_fee, status, created, refund_fee, oid, good_status, company_name, sid, payment, reason, desc, has_good_return, modified, order_status';
    /**
     * 定义API每页获取的订单数
     *
     * @var Integer
    ...
    private function getFullTradeInfo($tid,$shop_id='') {
        ...
        if (is_array($apiResult) && !isset($apiResult['error_response']) && !empty($apiResult['trade_fullinfo_get_response']['trade'])) {
            ...
            $result = array('status'=>'fail','msg'=>'timeout');
        }elseif(isset($apiResult['error_response']) && isset($apiResult['error_response']['code'])){
        	$result = array('status'=>'fail','msg'=>$apiResult);
     ...
     * 
     */
    public function sendGoods($tid,$shop_id=''){
    	...
    	$result = array();
    	if (is_array($apiResult) && !isset($apiResult['error_response']) && $apiResult['delivery_dummy_send_response']['shipping']['is_success']) {
            $result = array('status'=>'succ','msg'=> 'success');
        }elseif(isset($apiResult['error_response']) && isset($apiResult['error_response']['code'])){
            $result = array('status'=>'fail','msg'=>$apiResult);
        }elseif(empty($apiResult)){
            $result = array('status'=>'fail','msg'=>'timeout');
    ...
    public function getIncrementOrdersByPage($startTime,$endTime,$page=1,$page_size=1,$shop_id='') {
        ...
        if (empty($this->sessionKey)) {
            ...
            return array('status'=>'fail','msg'=>'no session');
        }
    ...
    //获取退款详情
	public function getRefundOrdersByPage($startTime,$endTime,$buyer_nick,$status,$page=1,$page_size=1,$shop_id='') {
        ...
        $params = array(
            'fields' => self::FIELDS_REFUND, //self::FIELDS,
            'page_size' => $page_size,
            'start_modified' => $this->aliStartDay,
            'end_modified' => $this->aliEndDay
        );
		
        if(isset($buyer_nick) && !empty($buyer_nick)){
        ...
        if (is_array($apiResult) && !isset($apiResult['error_response']) && !empty($apiResult['refunds_receive_get_response']['refunds']['refund'])) {
            ...
            $result = array('status'=>'fail','msg'=>$apiResult['error_response']);
        }elseif(empty($apiResult['refunds_receive_get_response']['refunds']['refund'])){
            $result = array('status'=>'fail','msg'=>'Order is empty');
        }
        return $result;
    ...
    public function transOrderDetail($order) {
        ...
        */
        ...
        $omeOrder = array(
        	...
        	'order_type' => 'normal',//normal（普通订单）,agentsale（寄售订单）
            //'memeber_id' => $order['seller_nick'] ? $order['seller_nick'] : '',
            'status' => strval($this->_status($order['status'])),
            'pay_status' => strval($this->_payStatus($order['status'])),
            'ship_status' => strval($this->_shipStatus($order['status'])),
            //'is_delivery' => '',
            ...
            */
            'pmt_goods' => '0',
            'pmt_order' => strval(abs(floatval($pmt_order))),
            'total_amount' => $order['payment'],
            'cur_amount' => $order['payment'],	//实际货币所需付款，与订单总金额一致
