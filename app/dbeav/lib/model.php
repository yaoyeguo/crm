...
class dbeav_model extends base_db_model{
    ...
    function count($filter=null)
    {
        ...
        $sql = 'SELECT count(*) as _count FROM `'.$this->table_name(1).'` WHERE '.$this->_filter($filter);
    ...
    function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null, $forceIndex=''){
        if(!$cols){
        ...
        $sql = 'SELECT '.$cols.' FROM `'.$this->table_name(true).'` '.$forceIndex;
