...
class market_finder_api_log{
    ...
    function detail_log($log_id){
        ...
        $oApilog = &app::get('market')->model("api_log");
        ...
        if (is_array($apilog_msg)){
            ...
            $msg = $apilog['msg'];
            $code_msg = market_api_func::api_code2msg($msg, $log_id);
            if (!empty($code_msg)){
                $msg = $code_msg;
            }
        }
    ...
    function column_retry($row){
        ...
        $button = "<a class=\"btn\" href=\"index.php?app=market&ctl=admin_api_log&act=retry&p[0]={$log_id}&finder_id={$finder_id}\" target=\"dialog::{title:'日志重试', width:550, height:300}\">重试</a>";
