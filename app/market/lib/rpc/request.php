...
class market_rpc_request {
    ...
    protected function fetchShopInfo($shopId) {
        return app::get(ORDER_APP)->model('shop')->dump(array('shop_id' => $shopId), '*');
    }
    ...
    public function request($method,$params,$callback=array(),$title,$shop_id=NULL,$time_out=1,$queue=false,$addon=''){
        ...
        if($callback && $callback['class'] && $callback['method']){
            ...
            $rpc_callback = array('market_rpc_request','callback',array('log_id'=>$log_id));
        }
        ...
        if ($queue == true){
            ...
            $oApi_log->write_log($log_id,$title,'market_rpc_request','rpc_request',array($method, $api_params, $rpc_callback));
    ...
    public function rpc_request($method,$params,$callback,$time_out=5){
        ...
        $rst = app::get('market')->matrix()->set_callback($callback_class,$callback_method,$callback_params)
    ...
    private function _check_node($shop_id,$method){
        ...
        if($node){
            $request_whitelist = kernel::single('market_rpc_request_whitelist');
            $t_node = $node;
    ...
    private function _check_request_config($shop_id, $method) {
        ...
        if ($method == 'store.items.quantity.list.update') {
            ...
            $request_auto_stock = app::get('market')->getConf('request_auto_stock_' . $shop_id);

            //如无设置,缺省置为 true
            if (empty($request_auto_stock)) {
                $request_auto_stock = 'true';
                app::get('market')->setConf('request_auto_stock_' . $shop_id, 'true');
            }
    ...
    function run(&$cursor_id,$params){
        ...
        $oApi_log->write_log($log_id,$title,'market_rpc_request','rpc_request',array($method,$api_params,$rpc_callback));
        kernel::single('market_rpc_request')->rpc_request($method,$params,$rpc_callback);
