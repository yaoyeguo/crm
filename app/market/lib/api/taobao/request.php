...
abstract class market_api_taobao_request extends market_api_abstract {
	//API访问地址
	...
	/*protected function getSessionKey() {
	 ...
	 $db = $this->getDB();
	 $res = $db->query("SELECT * FROM sdb_base_kvstore WHERE prefix='market' AND `key`='taobusession'");
	...
	protected function getBaseApiRequestParam() {
		...
		if($_SERVER['SERVER_NAME'] == '159140332.taojixiao.taoex.com'){
			$result = array('app_key' => '10011902',
    ...
    protected function apiRequest($method, $params,$shop_id = '') {
		...
		ksort($params);
		if($_SERVER['SERVER_NAME'] == '159140332.taojixiao.taoex.com'){
			$signString = '2fc426b0109908169017efb33a71f15c';
		}else{
		...
		if($_SERVER['SERVER_NAME'] == '159140332.taojixiao.taoex.com'){
			$result = market_api_httpclient::quickPost('http://gw.api.matrix.shopex.cn/router/rest', $params);
        }else{
			//$result = market_api_httpclient::quickPost(self::SIP_URL, $params);
            $result = $this->get_api($params,$shop_id);
        }


		$apiResult = $this->xml2array($result);

		if (is_array($apiResult) && isset($apiResult['error_response'])){
			ilog(var_export($params,true));
			ilog(var_export($apiResult,true));
		}
