<?php
class market_ctl_site_weixin_ucenterNew extends base_controller{
    ...
    public function gifts()
    {
        ...
        $this->display('site/weixin/ucenter/gifts_new.html');
    }
    ...
    public function order_confirm()
    {
        ...
        if($_POST){
            ...
            //创建订单
            if($_POST['new_addr']['area_str']){
                //area的值有两种，130000/130300/130303:河北省/秦皇岛市/山海关区 ; 310100/310103:上海市/卢湾区
                $address = explode(':',$_POST['new_addr']['area_str']);
                $address_v = explode('/',$address[1]);
                if(count($address_v) < 3){
                    $state = $address_v[0];
                    $city = $address_v[0];
                    $area = $address_v[1];
                }else{
                ...
                $addr = implode(',', $address_v).$_POST['new_addr']['addr'];
            }
            ...
            $order = array(
                ...
                'mobile' => trim($_POST['mobile']),
                'state' => $state,//trim($_POST['new_addr']['state']),
                'city' => $city,//trim($_POST['new_addr']['city']),
                'area' => $area,//trim($_POST['new_addr']['district']),
                'addr' => $addr,
        ...
        foreach((array)$rs_orders as $k=>$v){
            ...
            $ship_area_arr = explode(':',$v['ship_area']);
            $addr_info_arr = explode('/',$ship_area_arr[1]);
            $rs_orders[$k]['state'] = $addr_info_arr[0];
            $rs_orders[$k]['city'] = $addr_info_arr[1];
            $rs_orders[$k]['district'] = $addr_info_arr[2];
        }*/
        //获取默认收货地址
        $addrObj = app::get('taocrm')->model('member_receivers');
        if($receiver_id){
            $rs_orders = $addrObj->db->selectRow('SELECT * FROM sdb_taocrm_member_receivers WHERE receiver_id='.$receiver_id);
        }else{
            $rs_orders = $addrObj->db->selectRow('SELECT * FROM sdb_taocrm_member_receivers WHERE member_id='.$wxMember['member_id'].' and default_addr="true"');
        }

        $area = explode(':',$rs_orders['area']);
        $rs_orders['area_show'] = str_replace('/','',$area[1]);

        //微信openid
        $rs_orders['wx_id'] = $this->wx_id;
        $this->pagedata['member_id'] = $wxMember['member_id'];
        $this->pagedata['gifts'] = $rs_gifts;
        $this->pagedata['rs_orders'] = $rs_orders;
        $this->pagedata['select_href'] = '../site_weixin_member/select_receiving_address?wx_id='.$rs_orders['wx_id'].'&gift_id='.$rs_gifts['buy_id'].'&rand_num='.time();
        $this->display('site/weixin/ucenter/order_confirm_new.html');
    }
    ...
    public function order_submit_ajax(){
        ...
        if(!empty($rs_gifts) && $rs_gifts['limit_times'] != 'Unlimited'){
            if($rs_gifts['limit_times'] <= $do_times){
                echo('非常抱歉，每人仅限'.$rs_gifts['limit_times'].'次，请关注我们的更多活动~');
                exit;
            }
        }
        //积分是否够用
        if($this->rs_member['all_points'] < $rs_gifts['minus_score']){
            echo('非常抱歉，您的积分不够，请关注我们的更多活动~');
            exit;
        }
        echo 'succ';
        exit;
