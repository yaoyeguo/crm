...
class market_ctl_admin_callcenter_workspace extends desktop_controller{
    ...
    {
        if($_GET['tab'] == 'callplan_members'){
            $this->callplan_members();
            exit;
        }
        $param=array(
            'title'=>'呼叫计划',
    ...
    public function callplan_members()
    {
        $param=array(
            'title'=>'我的呼叫列表',
            'use_buildin_recycle'=>false,
    ...
    function _views()
    {
        $count = $this->app->model('callplan')->count();
        $sub_menu[] = array(
            'label'=> '呼叫计划',
            'filter'=> array(),
            'optional'=>false,	
            'addon'=>$count,	
            'href' => 'index.php?app=market&ctl=admin_callcenter_workspace&act=index&view=0',
        );
        
        $filter = array(
            'assign_user_id'=>$this->user_id,
        );
        $count = $this->app->model('callplan_members')->count($filter);
        $sub_menu[] = array(
            'label'=>'我的呼叫列表',
            'filter'=>$filter,
            'optional'=>false,
            'addon'=>$count,
            'href' => 'index.php?app=market&ctl=admin_callcenter_workspace&act=index&tab=callplan_members&view=1',
        );
