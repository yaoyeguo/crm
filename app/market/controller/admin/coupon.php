<?php
class market_ctl_admin_coupon extends desktop_controller{
...
//        }
        $this->finder('market_mdl_coupons',array(
            ...
            'finder_cols' => 'column_edit,shop_id,coupon_name,status,created,end_time',
        ));
    }
    ...
    public function _views(){
        ...
        $sub_menu[] = array(
            'label'=> '可兑换',
            'filter'=> array('is_exchange'=>1, 'shop_id|in' => $shops),
            'optional'=>false,	
        );
    ...
    public function _editor($coupon_id=NULL){
        if($coupon_id && $coupon_id>0){
            ...
            $this->pagedata['coupon'] = array('status'=>1,'start_time'=>date('Y-m-d'));
        ...
        $shopObj = &app::get(ORDER_APP)->model('shop');
        $shopList = $shopObj->getList('shop_id,name',array('node_type'=>'taobao'));
    ...
    public function toAdd() {
        ...
        if($_POST['start_time']){
            $coupon['start_time'] = strtotime($_POST['start_time'].' '.$_POST['_DTIME_']['H']['start_time'].':'.$_POST['_DTIME_']['M']['start_time'].':00');
        }
        ...
        //if( ($active_id = kernel::single('market_service_activity')->saveActivity($activity)) ){
            ...
            if( ($coupon_id = kernel::single('market_service_coupon')->saveCoupon($coupon)) ){
                if(!kernel::single('market_service_coupon')->requestCoupon($coupon_id,$msg)){
                    ...
                    $this->end(false,'请求优惠券接口失败:'.$msg);
                }

                if(!kernel::single('market_service_activity')->requestActivity($coupon_id,$msg)){
                    $this->_applyAuth($msg,$coupon['shop_id']);
                    $this->end(false,'请求优惠券活动接口失败:'.$msg);
                }

                $this->end(true,'操作成功');
            }else{
                $this->end(true,'保存优惠券失败');
            }
        /*}else{
    ...
    private function _applyAuth(& $msg,$shopId,$jumpto = 'coupon_goOnAdd'){
        if(strstr($msg, 'Insufficient session permissions') || strstr($msg, 'Insufficient security level:W2 security authorize invalid')){
            $db = kernel::database();
            $db->rollback();
