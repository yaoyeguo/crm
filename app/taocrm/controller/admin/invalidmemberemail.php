<?php
class taocrm_ctl_admin_invalidmemberemail extends desktop_controller{
	...
	public function index()
	{
		...
		$title = '无效邮箱客户列表';
		$actions = '';
		$baseFilter = array('invalidType'=>'email');
	...
	public function _views()
	{
		...
		foreach($sub_menu as $k => $v){
			...
			if (!isset($_GET['view']) && $count > 0) {
				$this->redirect('index.php?app=taocrm&ctl=admin_invalidmemberemail&act=index&view='. $i++);
				exit;
			}
	...
	protected function getDBAllShopInfo()
	{
		...
		$filter = array('status'=>0,'invalidType'=>'email');
