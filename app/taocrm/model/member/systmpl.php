...
class taocrm_mdl_member_systmpl extends dbeav_model{
    ...
    function fetch($tplname,$data=null){
        ...
        if(count($aTmpl) != 1){
            ...
            $app_id = $aLast[0];
			$obj_app_id = kernel::service('taocrm_messenger_tpl_appid');
			if ($obj_app_id && method_exists($obj_app_id, 'get_app_id')){
				$obj_app_id->get_app_id($aTmpl[2], $app_id);
			}
            $aLast[0] = 'admin';
            $dir = implode('/',$aLast);
    ...
    function _file($name){
        if($p = strpos($name,':')){
            ...
            return ROOT_DIR.'/app/taocrm/view/'.$name.'.html';
        }
    }
    ...
    function clear($name,&$msg=''){
        $sdf = $this->dump($name);
		if (!$sdf) {
			$msg = app::get('taocrm')->_('打印样式未保存过，无需恢复！');
			return false;
		}
        $sdf['edittime'] = time();
        $sdf['active'] = 'false';
