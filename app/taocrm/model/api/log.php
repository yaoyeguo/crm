<?php
class taocrm_mdl_api_log extends dbeav_model{
    ...
    function retry($log_id='', $retry_type='', $isSelectedAll='', $cursor='0'){
        ...
        if ($retry_type=='batch' and ( strstr($log_id,"|") or $isSelectedAll == '_ALL_' ) ){
            ...
            //单个按钮重试
            $row = $this->db->selectrow("SELECT * FROM sdb_taocrm_api_log WHERE log_id='".$log_id."' and status='fail' ");
    ...
    function start_api_retry($row){
        ...
        $status = 'fail';
        $msg = '手动重试';
        $return = $this->db->exec("UPDATE sdb_taocrm_api_log SET retry=retry+1,last_modified='".time()."',status='sending',msg='".$msg."' WHERE log_id='".$log_id."'");
