...
class taocrm_analysis_task {
    public function analysis_day(){
        $data = $this->fetch_by_interval('day');
        foreach(kernel::serviceList('taocrm_analyse_day') AS $service){
            if(!$service instanceof taocrm_analysis_interface) continue;
            $run_task = false;
            ...
            if(!isset($data[$service_name])){
                ...
                if($analysis_id = app::get('taocrm')->model('analysis')->insert($new_service)){
                    $run_task = true;
                }
            ...
            if($run_task){
                ...
                if($rows){
                    foreach($rows AS $row){
                        ...
                        $logs['time'] = $run_day;
                        app::get('taocrm')->model('analysis_logs')->insert($logs);
                    }
                }
                app::get('taocrm')->model('analysis')->update(array('modify'=>$run_day), array('id'=>$analysis_id));
            }
        }
    }//End Function
    ...
    public function analysis_hour(){
        ...
        foreach(kernel::serviceList('taocrm_analyse_hour') AS $service){
            if(!$service instanceof taocrm_analysis_interface) continue;
            $run_task = false;
            $service_name = get_class($service);
            ...
            if($run_task){
                ...
                if($rows){
                    foreach($rows AS $row){
                        ...
                        $logs['value'] = $row['value'];
                        $logs['time'] = $run_hour;
                        app::get('taocrm')->model('analysis_logs')->insert($row);
                    }
                }
                app::get('taocrm')->model('analysis')->update(array('modify'=>$run_hour), array('id'=>$analysis_id));
            }
        }
    }//End Function

    public function fetch_by_interval($interval){
        $rows = app::get('taocrm')->model('analysis')->getList('*', array('interval'=>$interval));
        foreach($rows AS $row){
