<?php
class taocrm_analysis_member_buyprice extends taocrm_analysis_abstract implements taocrm_analysis_interface{
    ...
    public function get_logs($time){
        ...
        $tmp = $logObj->get_member_avg(array(
            'start_time' => $date,
            'end_time' => $date
        )) ;
        
        foreach($tmp as $k=> $v){
            $result[] = array('type'=>0, 'target'=>1, 'flag'=>$shop[$v['shop_id']], 'value'=>$v['AVG']);
        }
