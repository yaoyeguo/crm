...
class orderManager {
     ...
     * @var String
     */
    private $script = S_ORDER_QUEUE_EXEC_SCRIPT;
    ...
    public function exec() {
        ...
        for(;;) {
            //执行死循环
            while (($this->threadRunning < $this->maxThread) && ($redis->LLEN(S_ORDER_QUEUE) > 0)) {

                echo sprintf("Total %s orders in queue\n", $redis->LLEN(S_ORDER_QUEUE));
                $this->running[] = new thread ($this->executable, $this->script, ' ', 20);
                $this->threadRunning++;
            }

            //检查是否已经结束
            if (($this->threadRunning == 0) && ($redis->LLEN(S_ORDER_QUEUE) == 0)) {
