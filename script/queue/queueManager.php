...
class queueManager {
     ...
     * @var String
     */
    private $script = S_QUEUE_EXEC_SCRIPT;
    ...
    private $normal_key = S_NORMAL_QUEUE;

    private $realtime_key = S_REALTIME_QUEUE;
    ...
    public function exec() {
        ...
        for(;;) {
            ...
            while (($this->threadRunning < $this->maxThread) && ($redis->LLEN($this->realtime_key) > 0 || $redis->LLEN($this->normal_key) > 0)  ) {

                echo sprintf("Total %s queue\n", ($redis->LLEN($this->realtime_key) + $redis->LLEN($this->normal_key)));
                $this->running[] = new thread ($this->executable, $this->script, ' ', 20);
                $this->threadRunning++;
            }

            //检查是否已经结束
            if (($this->threadRunning == 0) && ($redis->LLEN($this->realtime_key)==0 && $redis->LLEN($this->normal_key) == 0) ) {
